...
class HeaderMap[B >: String] private(map: Map[CaseInsensitiveString, B] = Map())
 // extends WithDefault[String, B](new HeaderMap[B](), Map()) {
 // extends MapOps[String, B, Map[String, B], HeaderMap[B]] {
  ...
  override def updated[B1 >: B](key: String, value: B1): HeaderMap[B1] = {
    new HeaderMap[B1](map.updated(new CaseInsensitiveString(key), value))
  }
